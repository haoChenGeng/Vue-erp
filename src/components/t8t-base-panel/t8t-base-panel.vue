            ...
            <el-col :span="6"
                ...
                <el-form-item :label="field.label?field.label+'：':''"
                    ...
                    <t8t-form-popup
                        ...
                        v-if="field.type==='popup'"
                        v-model="dataSource[field.prop]"
                        :bindValue="dataSource[field.prop]"
                        ...
                        :dialog="field.dialog"
                        :customItem="field.customItem"
                        :fetchSuggestions="field.fetchSuggestions"
                        :popperClass="field.popperClass"
                        :triggerOnFocus="field.triggerOnFocus"
                        :options="(field.selectSourceKey&&selectSource)?selectSource[field.selectSourceKey]:null"
                        :remote="field.remote"
                        :service="field.service"
                        :method="field.method"
                        :remoteArgs="field.remoteArgs"
                        :remoteQueryKey="field.remoteQueryKey"
                        :autoText="field.autoText"
                        :autoValue="field.autoValue"
                        :useQuery="field.useQuery"
                        @select="popupOnSelect"
                        @change="popupOnChange"
                    >
                    </t8t-form-popup>
                    <el-input v-else-if="field.type==='input' || field.type==='textarea'"
                    ...
                    <el-select
                        v-else-if="field.type==='select-option-group'"
                        v-model="dataSource[field.prop]"
                        :disabled="(mode === 'view') ? true : (mode !== 'view' && !disabled && field.disabled)"
                        @change="onElSelectChange(arguments[0], field.prop)"
                        :name="field.prop"
                        placeholder="请选择">
                        <el-option-group
                            v-for="item in options[field.list]"
                            :label="item.label">
                            <el-option
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            popupOnSelect (value, prop) {
                this.$emit('popup-on-select', value, prop)
            },
            popupOnChange (value, prop) {
                this.$emit('popup-on-change', value, prop)
            },
