<template>
    <div class="app-content-container" :class="{'stretch':stretch}">
        <div class="erp-tabs" id="erp-tabs">
            <div class="left-slide slide-chunk" @click="scrollLeft(-116)">
                <i class="el-icon-caret-left"></i>
            </div>
            <div id="g-item-wrapper" class="item-wrapper">
                <div id="g-item-container" class="item-container clearfix">
                    <el-tooltip v-for="(item, index) in tabs" effect="dark" popper-class="tabs-tooltip" :content="item.route.meta.title"
                                placement="bottom">
                        <router-link :to="item.route.fullPath" tag="div" class="router-link">
                            <span class="router-link-span">{{item.route.meta.title}}</span>
                            <i class="el-icon-circle-close" @click.stop="popTab(index)"></i>
                        </router-link>
                    </el-tooltip>
                </div>
            </div>
            <div class="right-slide slide-chunk" @click="scrollLeft(116)">
                <i class="el-icon-caret-right"></i>
            </div>
            <div class="close-all slide-chunk" @click="closeAllTabs">
                <i class="el-icon-circle-cross"></i>
            </div>
        </div>
        <!-- 右键菜单 -->
        <context-menu ref="headerMenu">
          <div class="menu-item" @click="memuHandler(1)">关闭全部标签页</div>
        </context-menu>

        <!-- 新页面 -->
        <keep-alive>
            <router-view v-if="$route.meta.isFulldialog===false" class="app-content"></router-view>
        </keep-alive>
    ...
    export default {
        ...
        methods: {
            ...
            needCache (route) {
                ...
                // 新页面
                if (route.meta.isFulldialog === true || route.meta.isFulldialog === false) {
                    return !route.meta.isFulldialog
                }
                // 旧页面
                return route.matched.every((match) => {
                    // 全屏弹窗和404这种非业务类型不需要被缓存
                    if (match.components && match.components.popView || (match.path.indexOf('/forbidden') !== -1)) {
                        return false
                    }
                    return true
                })
            ...
            scrollIntoView () {
                setTimeout(function () {
                    let obj = document.querySelector('#g-item-container .router-link-active')
                    obj && obj.scrollIntoViewIfNeeded && obj.scrollIntoViewIfNeeded()
                }, 0)
            },
            closeAllTabs () {
