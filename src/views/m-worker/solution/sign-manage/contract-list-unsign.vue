    ...
    export default {
        ...
        data () {
            return {
                ...
                tableArgs: { search: {id: null, orderStatus_ne: 7}, userId: Cookie.get('t8t-tc-uid'), sort:["id_desc"] },
                // 表头描述
                tableColumns: [
                    {prop: 'yid', label: '项目ID'},
                ...
                // 路由地址
                viewPaperRouter: '/tuchat-projectlist/preview',
                viewSelectionRouter: '/tuchat-worker/quote-order-detail',
                addRouter: '/tuchat-worker/sign-manage/add'
            }
        },
        created() {
            this.mainActiveTab = this.$route.query.tab ? this.$route.query.tab : 'unsign'
        },
        methods: {
            // 搜索
            submitSearch(obj) {
                // 查出非已签约的数据
                obj.orderStatus_ne = 7
            ...
            add()
            {
                this.$TCS.addTag('erp_scm_018003006003');
                const row = this.getLastSelectRow()
                if (row) {
                    this.$router.push({
                        path: this.addRouter,
