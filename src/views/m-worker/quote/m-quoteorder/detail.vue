...
<template>
    <div class="" v-loading.fullscreen.lock="isLoadingAmount" element-loading-text="正在预估用量，请稍候 ...">
        ...
        <el-dialog size="full" class="my-el-dialog t8t-full-dialog quoteOrderDetailDialog" v-model="quoteOrderDialogVisible" @close="closeQuoteOrderDialog" :show-close="false" :close-on-press-escape="false">
            ...
            <div class="full-dialog-toolbar-container">
                ...
                <div class="toolbar-container">
                    <div>
                        <el-button @click="closeQuoteOrderDialog()">返回</el-button>
                    </div>
...
-->
                                                ...
                                                <div class="space-plus">
                                                    <el-button type="primary" size="mini" class="add-individuation" @click="openSelectQuoteTempItemDialog(index)" :disabled="!isCanEdit()"><i class="el-icon-plus el-icon--left"></i>个性化</el-button>
                                                    <el-button type="primary" size="mini" class="add-individuation" @click="syncAmount(index)" :disabled="!isCanEdit()"><i class="el-icon-setting el-icon--left"></i>预估用量</el-button>
                                                </div>
                                            </div>
                                        </div>
                                    </el-col>
                                        ...
                                        >
                                            <template scope="scope" slot="qiName" :class="cell-text">
                                                ...
                                                <div class="cell-text" style="float: left;">
                                                    <el-button type="danger" size="mini" class="remove-spaceitem" @click="removeQuoteItem(scope.row.qiName, scope.row, index, scope)" title="删除报价项" v-if="scope.row.tempItemType != 0 && scope.row.dataType == 1" :disabled="!isCanEdit()"><i class="el-icon-delete el-icon&#45;&#45;left"></i>删除</el-button>
                                                </div>
...
<script>
    ...
    export default {
        ...
        components: {selectQuoteTempItemDialog,selectSkuDialog,selectQuoteTemp},
        created() {
            this.initParams()
        ...
        methods: {
                                            ...
                                            spaceSizeForms[tableIndex].validate((isValid3) => {
                                                ...
                                                if(isValid3){
                                                    ...
                                                    if( tables.length == tableIndex+1 ){
                                                        ...
                                                        this.validateData(tables, spaceAliasForms, spaceSizeForms,tableIndex+1)
                                                    }
            ...
            tempSave: function (){
                ...
                this.validateData(tables, spaceAliasForms, spaceSizeForms, 0)
            },

            //暂存报价单(新建及编辑)
            tempSaveDo: function (){
                this.$refs['topToolbar'].disableBySymbol('TEMP-SAVE')
                ...
                methods.saveQuoteOrder(data,+Cookie.get('t8t-tc-uid')).then((res) => {
                    if(res.data.status == 200){
                        ...
                        this.$refs['topToolbar'].unDisableBySymbol('TEMP-SAVE')
                        this.$msgbox({
                            title:'',
                            type: 'error',
            ...
            reset: function (){
                this.$confirm('新建报价单，当前报价单将被清空，当前所有报价项信息将丢失，请确定是否新建。','提示',{
                    ...
                    this.$refs['topToolbar'].disableBySymbol('RESET')
                    methods.newCreateQuoteOrder(this.quoteOrderInfo.id, +Cookie.get('t8t-tc-uid')).then((res) => {
                        ...
                        if( res.data.status == 200 ){
                            ...
                            this.$refs['topToolbar'].unDisableBySymbol('RESET')
            ...
            preview: function (){
                this.$router.push({path:'/worker/quote-order-preview',query:{tempId:this.tempId, yid: this.yid,orderId:this.quoteOrderInfo.id}})
            },
            ...
            submit: function (){
                ...
                this.$confirm('提交审核前请确认报价单信息完整无误。是否继续提交审核？','提示',{
                    ...
                    if( _this.quoteOrderInfo.hasOwnProperty('id') ){
                        ...
                        if( JSON.stringify(this.spacesDataClone) != JSON.stringify(this.spacesData) ){
                            this.$confirm(
                                '有数据变更，继续提交未保存的数据将丢失，是否继续提交审核？',
                                '提示',
                                {
                                    type: 'warning',
                                    confirmButtonText: '继续提交',
                                ...
                                _this.$refs['topToolbar'].disableBySymbol('EDIT-SUBMIT')
                                methods.submitQuoteOrder(_this.quoteOrderInfo.id, +Cookie.get('t8t-tc-uid')).then((res) => {
                                    if( res.data.status == 200 ){
                                        ...
                                        _this.$refs['topToolbar'].unDisableBySymbol('EDIT-SUBMIT')
                                    }
                                })
                            }).catch(() => {
            ...
            getSpaceOrderItemList: function (){
                ...
                for (let i in  data){
                    ...
                    serialNumber += temp_data.quoteOrderItemList.length
            ...
            formatSpaceDataToSave(data, serialNumber){
                ...
                data.quoteOrderItemList.forEach((item, index) => {
                    ...
                    if( item.dataType == 2 || this.isEmpty(item.gdmId)){
                        ...
                        newData.items.push({
                            ...
                            "serialNumber": index+1+serialNumber, //报价单行展示序号，从1开始
                            "amount": amount, //数量
            ...
            handleCommandQuoteItem(data){
                ...
                //同一空间中不能选择相同的报价项
                let tempItemId = data.tempItemId

                let sameTempItem = quoteOrderItemList.filter(i => {return i.tempItemId == tempItemId})
            ...
            initQuoteOrderDetail: function (){
                ...
                methods.initQuoteOrderDetail(this.yid, this.tempId, this.area, this.houseTypeId, +Cookie.get('t8t-tc-uid')).then(res => {
                    ...
                    if( [0,2,5].indexOf(quoteOrder['orderStatus']) > -1 ){
                        _topToolbar.unDisableBySymbol('RESET')
                    }
                    ...
                    if( [0,2,5].indexOf(_this.quoteOrderInfo['orderStatus']) > -1 || !_this.quoteOrderInfo.hasOwnProperty('id') ){
                        _topToolbar.unDisableBySymbol('TEMP-SAVE')
                    }
                    //“报价完成” 可以点击反审核
                    if( [6].indexOf(_this.quoteOrderInfo['orderStatus']) > -1 ){
                        _topToolbar.unDisableBySymbol('ANTI-AUDITING')
                    }
                    ...
                    if( _this.quoteOrderInfo.hasOwnProperty('id') ){
                        _topToolbar.unDisableBySymbol('PREVIEW')
                    }
            ...
            * 格式化空间及报价项数据
            ...
            formatDataForSpacesData(item, isParent = false){
                ...
                if( item.rows != undefined ){
                    ...
                    dataMap.forEach(i => {
                        ...
                        //根据是否有组合用量关系id(gdmId)判断报价项是否有配置组合用量关系， 没有配置的时候不需要创建选品行
                        if( !this.isEmpty(qiData.gdmId) ){
                            i.forEach((i1,index) => {
                                i1.mainIndex = mainIndex

                                //选品行的名称取的是组合别名
                                i1.qiName = i1.gdmName ? i1.gdmName : ''
                                datas = [].concat(datas, i1)
                            })
                        }
            ...
            removeQuoteItem: function(val, row, tableIndex, scope){
                let tempItemId = row.tempItemId
                let mainIndex = row.mainIndex
                let table = this.$refs.t8ttable[tableIndex]

                //删除报价项行
                table.dataSource.splice(scope.cell.$index,1)

                if(row.hasOwnProperty('id') && row.id != undefined){
                    this.delQuoteItemIds.push(row.id)
                }

                //同时需要删除报价项下的选品行
                let data = table.dataSource.filter(i => {
                    return i.mainIndex == mainIndex
                })
                ...
                if(data.length){
                    data.forEach(i => {
                        ...
                        if(i.hasOwnProperty('id') && i.id != undefined){
                            this.delQuoteItemIds.push(row.id)
                        }
                    })
                }
            ...
            getdirectProjectFee: function () {
                ...
                //直接工程费 = 标配套餐金额 + 个性化总额
                this.directProjectFee  = total+pkgPrice
            ...
            //是否可进行数据编辑
            isCanEdit: function (){
                //“完成初稿”、“主管审核驳回”、“核算审核驳回” 可以编辑
                return [0,2,5].indexOf(this.quoteOrderInfo['orderStatus']) > -1 || !this.quoteOrderInfo.hasOwnProperty('id')
            },
            ...
            recheck(){
                ...
                this.$confirm('请确认是否提交反审核','提示',{
                    ...
                    _this.$refs['topToolbar'].disableBySymbol('ANTI-AUDITING')
                    methods.recheckQuoteOrder(_this.quoteOrderInfo.id, +Cookie.get('t8t-tc-uid')).then((res) => {
                        if( res.data.status == 200 ){
                            ...
                            _this.$refs['topToolbar'].unDisableBySymbol('ANTI-AUDITING')
                        }
                    })
                })
            }
