<template>
	<div>
        ...
        <p><span v-if="photoesStatus == 2">（已完成）</span><span v-else>（未完成）</span></p>
		<div class="picture-list">
			<div class="list-item" v-for="(item,index) in fileList">
				...
				<div class="item-text">
					<p>
                        {{ item.photoName }}
                        {{ getTime(item.createTime, item.updateTime) }}
					</p>
		...
		<div>
			...
			<p>2、图片类型文件只允许上传jpg，jpeg，png，bmp格式</p>
		</div>
		<div class="picture-list">
			<div class="list-item">
				<div class="item-img">
					<img src="./11.jpg" width="100%" height="100%" @click="showPicture1($event.target.src)">
				</div>
				<div class="item-text">
					<p>
						橱柜设计图（样例）
...
<script type="text/javascript">
	...
	export default {
		data(){
			return {
		 		breadcrumbData: [{ title: '木作图' }, { title: '木作图' }],
		...
		created(){
			this.getFileList();
			this.getTypeList({ pPropertyCode: 30103});
		},
		methods:{
		    getTypeList(args){
		        let arg =  {
			...
			getFileList(){
				axios({
					...
					args:{ photoesId: this.photoesId, sort:[ 'photoeType_desc' ]}
				}).then((res)=>{
					if(res.data.status === 200){
            ...
            beforeUpload(file){
            	if(this.acceptType == '.dwg'){
            		//dwg文件无法解析出type
            		if(file.size > 16 * 1024 * 1024){
            			this.$message.error('dwg源文件不能超过16M！');
            			return false;
            		}
            	}else{ //图片上传限制
            		if(['image/jpg','image/x-png','image/png','image/bmp','image/jpeg','image/pjpeg'].indexOf(file.type) == -1){
            ...
            switchPhotoType(val){
                ...
                this.imageUrl = '';
                if (val === 1) { //上传图片
                    this.acceptType = '.jpg,.jpeg,.png,.bmp';
                } else if (val === 0) { //上传源文件
                    this.acceptType = '.dwg';
                }
            },
            getTime(createTime, updateTime) {
              if (updateTime > 0) {
                return DateUtils(updateTime* 1000, 'yyyy/mm/dd')
              } else {
                  return DateUtils(createTime* 1000, 'yyyy/mm/dd')
              }
            }
		}
	}
