...
<script>
    ...
    export default{
        ...
        created() {
           this.getDiscountData(this.$route.query.rowId)
        },
        mounted () {
        },
        methods:{
            ...
            getDiscountData(rowId) {
                ...
                cashDiscountApi.getOne(args)
                    .then((res) => {
                    if (res.data.status === 200) {
                        this.formData.templateName = res.data.result.name
                        this.formData.parValueType = res.data.result.parValueType
                        this.formData.privilegeAmount = this.changeEmpty(res.data.result.parValue)
            ...
            tempsave()
            {
                ...
                if(isValid)
                {
                        ...
                        var formData =  this.formData
                         ...
                         var args = {
                            'orderCreateDto':{
                                'templateId' : this.$route.query.rowId,
                            ...
                            cashDiscountApi.createOrder(args)
                                .then((res) => {
                                    if (res.data.status === 200) {
                                        // 新增成功
