<template>
  <div class="check-details">
    <el-dialog @close="closeDialog" v-model="isDialogShow" size="full" class="t8t-full-dialog2 dialog-new operate-view" :class="{'isHide': isTopHide}">
      <div class="t8t-full-dialog2-container">
        <!-- 顶部按钮区 -->
        <div class="full-dialog-toolbar-container">
          <div class="toolbar-container">

            <t8t-toolbar class="t8t-dark" @EDIT-SUBMIT="SubmitBtn" :symbolList="symbolList" ref="indexToolbar">
            </t8t-toolbar>
          </div>
        </div>
        ...
        <div class="dialog2-main-container">
          ...
          <div class="table-list-container">
            <el-tabs v-model="activeTabName" @tab-click="clickTab">

              <div class="table-list">
                <el-tab-pane label="工艺信息" name="changeContent" v-model="technologyInfo">
                  <el-form class="dialog2-form-container" ref="formRef" :rules="formRules" label-position="right" label-width="100px">
                  ...
                  <el-tabs type="card" v-model="editableTabsValue" closable @tab-remove="removeTab">
                    <el-tab-pane v-for="(item,index) in craftTabs" :label="item.title" :name="item.name" :key="index" class="create-title-tab">
                      <el-button size="small" @click="add(item.title,index)" class="add-pic-btn">
                        新增图片
                      </el-button>
                      <upload @blur="getBlur" @delete="deleteUpload" @deletePic="deleteUploadPic" :tabIndex="index" :key="index1" :fileList2="getPicUrl(item1.imageUrl,item1.id)" :index="index1" :item="item1" class="edit-upload" v-for="(item1,index1) in item.content">
                      </upload>
    ...
    <div id="picDiv">
      <t8t-gallery ref="gallery" :data="dialogImageUrl" v-model="dialogVisible">
      </t8t-gallery>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [{ props: 'moduleName', label: '模块名称' }, { props: 'url', label: '图片' }, { props: 'des', label: '描述' }]
    };
  },
  props: {},
  computed: {
  ...
  methods: {
    ...
    addTab(targetName) {
      ...
      let names = this.craftTabs.map((item, index) => {
        return item.title;
      });
      // console.log(names);
