            ...
            <div class="t8t-full-dialog2-container">
                ...
                <div class="dialog2-main-container" :class="{'isHide': isTopHide}">
                    ...
                    <div class="full-dialog-tabs-container">
                        ...
                        <div class="t8t-detail">
                            <el-tabs v-model="subActiveTab" class="t8t-detail">
                                ...
                                <el-tab-pane :label="subTables[0].label" :name="subTables[0].name" ref="subTab">
                    ...
                    <div class="full-dialog-tabs-container">
                        ...
                        <div class="t8t-detail">
                            <el-tabs v-model="thirdActiveTab" class="t8t-detail">

                                <el-tab-pane :label="thirdTables[0].label" :name="thirdTables[0].name">
                                    <div class="sub-container">

                                        <!--table-->
                                        <t8t-table
                                            :preLoad="false"
                                            :columns="thirdTables[0].fields"
                                            :service="thirdTables[0].service"
                                            :method="thirdTables[0].method"
                                            :args="thirdTables[0].args"
                                            :editable="(mode === 'edit') ? true : (mode !== 'view' && !disabled)"
                                            :ref="thirdTables[0].name"
                                            :commonData="options || {}"
                                            @selection-change="selectionChange"
                                            @row-double-click="rowDbClick"
                                            @current-row-change="currentRowChange"
...
<script>
    ...
    import schema from './voucherOrder-schema'
    import voucherOrderApi from 'src/services/finance/voucherOrder.js'
    import voucherSourceItem from './voucherSourceItem-list'
    ...
    export default {
        ...
        data () {
            return {
                ...
                thirdTables: schema.thirdTables,
                ...
                thirdActiveTab: '',
                // 点击关闭后的返回route
                goBackRoute: '/tuchat-finance/page-voucher-order',
                sourceMap: {},
                columns: {},
                data: ''
            }
        ...
        methods: {
                ...
                commonApi.queryByFunctionCode(arg)
                    .then((res) => {
                        if (res.data.status === 200) {
                            ...
                            this.options.organizationId = list
                        }
                    })
            },
            onTopHide () {
            ...
            showItemDialog (data) {
                this.$router.push({ path: this.sourceItemRoute , query: { voucherEntryId: data }})
            },
            /**
             * 初始化附表
             */
            ...
            initThirdTable () {
                let that = this, schema = this.thirdTables, fieldCache
                if (schema && schema.length > 0) {
                    schema.forEach((item, key) => {
                        // 附表页签初始化选中项
            ...
            loadMainTable () {
                ...
                if (mainTableSchema.service && mainTableSchema.method) {
                    ...
                    this.$message.error('没有传服务名和方法参数')
                    return false
                }
            },
            /**
...
//                                        self.schema[key].dataSource = response.result
            ...
            loadThirdTable () {
                ...
                let self = this
                let schema = this.thirdTables, args, service, method, temp, argsString
                        ...
                        if (subSchema.type === 'table') {
                            ...
                            axios({
                                ...
                                .then((res) => {
                                    let response = res.data
                                    if (response && response.status == 200) {
                                        self.thirdDataSource[key] = response.result
//                                        self.schema[key].dataSource = response.result
                                    }
                                })
                                .catch((res) => {
