            ...
            apiReceiptPlanOrder.queryPage(args)
                .then((res) => {
                    if (res.data.status === 200) {
                        ...
                        {
                            this.channelCode='TBTSK004'
                        }else if(this.formData.finOrgName.indexOf("彬讯")>=0)
                        {
                            this.channelCode='BXSK003'
                        }
                        let orderArgs = {
                            payOrder: {
                ...
                .then((res)=>{
                    if (res.data.status === 200) {
                        this.payOrderCode = res.data.result.payOrderCode
                        let codeID=(Math.random()+'').split('.')[1]
                        this.qrCode='OrderNo='+this.payOrderCode+'&QRCodeID='+codeID+'&BizContent=nonghang&BizType='+1
                        this.$nextTick(()=>{
