...
<script>
    ...
    import accInvokeTemplateDialog from './accInvokeTemplate-dialog.vue'
    import apiAccInvokeTemplate from 'src/services/finance/apiAccInvokeTemplate.js'

    export default {
        components: { accInvokeTemplateDialog },
        data () {
            return {
                // 面包屑
                breadcrumbData: [{title: '财务'}, {title: '财务基础配置'}, {title: '业务调用模板'}],
                // 详情页路由地址
                detailRoute: '/test/resume/view',
                // service 和 method 目前需要加密
                service: Service.ACC_ACCOUNT_CONFIG.name,
                method: Service.ACC_ACCOUNT_CONFIG.methods.INVOKE_QUERYPAGE,
                tableArgs: {
                    search: {
                        aa: null
                    },
                ...
                tableColumns: [
                    ...
                    {prop: 'businessModuleName', label: '业务模块', show: true},
                    {prop: 'orderTypeName', label: '单据类型', show: true},
                    ...
                    {prop: 'recordChildTypeName', label: '收支子类型', show: true},
                    {prop: 'sourceUnitName', label: '来源单位', show: true},
                    {
                        prop: 'rate',
                        label: '比例',
                        show: true,
                        formatter: function (val,row,col,tab) {
                            return row.convertRateChildren.toString() + ':' + row.convertRateParent.toString()
                        }
                    },
                    ...
                    {prop: 'convertRateChildren', label: '兑换比例分子', show: false},
                    {prop: 'convertRateParent', label: '兑换比例分母', show: false},
                ],
                // 搜索项对应值
                commonData: {
                    recordType: [
        ...
        created() {
            //读取辅助资料
            //业务模块
 //           this.getCommonOptions('1!110!11011!', 'userTypeCodeOptions')
            //账户类型
            //           this.getCommonOptions(' ', 'accountTypeCodeOptions')
        },
        methods: {
            //点击新增按钮
            btnAddClick() {
            ...
            btnEnableClick(){
                ...
                if (selections.length === 0) {
                    ...
                    this.$confirm('确认对所选数据进行启用处理？', '启用确认', {
                        ...
                        apiAccInvokeTemplate.enbleConfig(params).then((res) => {
            ...
            btnDisableClick(){
                ...
                if (selections.length === 0) {
                    ...
                    this.$confirm('确认对所选数据进行禁用处理？', '禁用确认', {
                        ...
                        params.updateName = Cookie.get('t8t-tc-username')
                        params.updateBy = Cookie.get('t8t-tc-username')
                        apiAccInvokeTemplate.disableConfig(params).then((res) => {
                            if (res.data.status === 200) {
                                let msg = ''
                                if(res.data.result.fail>0){
                                    msg = '。原因: '+ res.data.result.reasons
