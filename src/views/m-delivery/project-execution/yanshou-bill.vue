...
<script>
    ...
    export default {
        ...
        data () {
            let that = this;
            return {
                symbolList:['ADD','EDIT','VIEW','YUYUE','BIANGENGGUANJIA','UNDO','ACCEPTANCE','OPLOG'],
                treeData: [],
                treeProps: {
                    label: 'name',
                    children: 'children'
                ...
                args: { sort: ['id_desc'] ,search:{ status_in: [0,1], payAdvisorId: +Cookie.get('t8t-tc-uid') } },
                opLogColumns:[
        ...
        activated(){
            if(this.$route.path == '/tochat-delivery/yanshou-bill' && typeof this.$route.query.refresh !== 'undefined'){
                this.getTableData();
            }
        },
        methods: {
            ...
            submitSearch(obj) {
                if(!this.publicKey){
                    utils.getPublicKey((res) => {
                        if(res.data.status == 200){
                            this.publicKey = res.data.result.key;
            ...
            showDetail: function(){
                ...
                if (selections.length === 0) {
                    ...
                    this.$router.push({
                        path: '/tochat-delivery/yanshou-view',
            ...
            acceptance (){
                ...
                if (selections.length === 0) {
                    ...
                    this.$router.push({
                        path: '/tochat-delivery/yanshou-detail',
                        query:{ billCode: selections[0].billCode,id: selections[0].id}
                    });
                }
            },
