...
<script>
 ...
 import {Service, methods} from 'src/services/delivery/installation-schedule/Service.js'
	export default{
		...
		created(){
            if(this.type && this.type == 'replenish' && this.projectId){
                this.service = comServices.DEMAND.name
                this.method = comServices.DEMAND.methods.QUERYTOOKGOODS
                this.goodsArgs = {goodsSearchDTO:{projectId:this.projectId}}
            }else{
                this.service = Service.INSTALLATION.name
                this.method = Service.INSTALLATION.methods.queryInstallTask
            }
		},
		methods:{
            confirm(){
