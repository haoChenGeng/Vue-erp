                ...
                <el-tabs v-model="activeTabName">
                    <div class="my-container">
                        ...
                        <div class="select-container-center">
                            <el-form>
                                <div class="form-item-container">
                                    <el-form-item label="派单原因：" class="is-required" ref="reasonSelect" prop="reason" v-if="viewData.itemManager != '未分配'">
        ...
        data () {
            return {
                ...
                searchFields: [
                    ...
                    {type: 'select', label: '接单状态', name: 'acceptState', selectSourceKey: 'acceptStates'}
                ],
                searchDataSource: {},
                // 辅助资料配置
                commonOptionsConfig: {
        ...
        methods: {
            ...
            submitForm: function (symbol) {
                if (this.principalRow == null || this.principalRow['accountId'] == null) {
                    this.$message.error('请选择项目经理后再试！')
                    return
                }

                if (this.viewData['itemManager'] !== '未分配' && this.reason <= 0) {
                    this.showMsg('error', '请选择重新派单的原因！')
                    return
                }

                var rowId = this.principalRow['accountId']
                var type = this.viewData['typeId']
            ...
            searchSubmit: function (search) {
                if (JSON.stringify(search) !== '{}') {
                    if (search['acceptState'] === '') {
                        search['acceptState'] = 0
                    }
            ...
            loadPrincipal: function (projectId, search) {
                ...
                itemAssignServiceApi.listItemPrincipal(args).then((res) => {
                    if (res.data.status === 200) {
